...
/// target advances to the tip commit as ops are appended.
...
impl<'repo> Store<'repo> {
    ...
    pub fn create(&self, author: &Actor, ops: Vec<Operation>) -> Result<TaskId> {
        ...
        let commit_oid = self.write_commit(&envelopes, None, author, "create task")?;
    ...
    pub fn append(&self, id: &TaskId, author: &Actor, ops: Vec<Operation>) -> Result<()> {
        ...
        let commit_oid = self.write_commit(&envelopes, Some(tip), author, &message)?;

        self.repo
            .reference(&ref_name, commit_oid, true, "git-task: update")
    ...
    pub fn load(&self, id: &TaskId) -> Result<Task> {
        ...
        let mut oids = vec![tip];
        let mut cur = self.repo.find_commit(tip)?;
        while cur.parent_count() > 0 {
            let parent_oid = cur.parent_id(0)?;
            oids.push(parent_oid);
            cur = self.repo.find_commit(parent_oid)?;
        }
        oids.reverse();
        ...
        for oid in oids {
            ...
            let entry = tree
                .get_name(OPS_BLOB_NAME)
                .ok_or_else(|| anyhow::anyhow!("commit {oid} is missing {OPS_BLOB_NAME}"))?;
            let blob = entry
                .to_object(self.repo)?
                .into_blob()
            ...
            let mut batch: Vec<OpEnvelope> = serde_json::from_str(text)
